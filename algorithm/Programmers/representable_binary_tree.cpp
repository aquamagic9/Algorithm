...
bool checkRepresentableBinaryTree(string s){
    ...
    bool check1 = checkRepresentableBinaryTree(s.substr(0, s.size() / 2));
    bool check2 = checkRepresentableBinaryTree(s.substr(s.size() / 2 + 1, s.size() / 2));
...
vector<int> solution(vector<long long> numbers) {
    ...
    for (long long n : numbers) {
        string s = numToBinaryString(n);
        if (checkRepresentableBinaryTree(s))
            answer.push_back(1);
        else
            answer.push_back(0);
    }
